...
class AuraManager:
    ...
    def _create_result(self, r_type: ElementalReactionType, source: Element, target: Element, consume: float = 0.0) -> ReactionResult:
        ...
        return ReactionResult(reaction_type=r_type, category=category, source_element=source, target_element=target, multiplier=mult, gauge_consumed=consume)
    ...
    def _apply_ec_tick(self, owner: Any) -> None:
        ...
        if h and e:
            ...
            if hasattr(owner, "event_engine"):
                owner.event_engine.publish(GameEvent(event_type=EventType.ELECTRO_CHARGED_TICK, frame=get_current_time(), source=owner, data={"target": owner}))
    ...
    def _apply_burning_tick(self, owner: Any, dt: float, is_damage_frame: bool) -> None:
        ...
        if grass:
            ...
            if is_damage_frame and hasattr(owner, "event_engine"):
                owner.event_engine.publish(GameEvent(event_type=EventType.BURNING_TICK, frame=get_current_time(), source=owner, data={"target": owner}))
