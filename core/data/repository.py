...
class MySQLDataRepository(DataRepository):
    ...
    def get_all_artifact_sets(self) -> List[str]:
        ...
        try:
            rows = self.db.execute_query(
                "SELECT Name FROM `artifact` ORDER BY Name ASC"
            )
            return [row[0] for row in rows]
        except Exception:
    ...
    def get_character_base_stats(self, character_id: int, level: int) -> Dict[str, Any]:
        ...
        char_info = self.db.execute_query(
            f"SELECT Name, Element, type FROM `character` WHERE ID = {character_id}"
        )
        if not char_info:
            raise ValueError(f"Character ID {character_id} not found.")
        name, element, ctype = char_info[0]

        hp = self.db.execute_query(
            f"SELECT `{col}` FROM `basehp` WHERE ID = {character_id}"
        )[0][0]
        atk = self.db.execute_query(
            f"SELECT `{col}` FROM `baseatk` WHERE ID = {character_id}"
        )[0][0]
        df = self.db.execute_query(
            f"SELECT `{col}` FROM `basedef` WHERE ID = {character_id}"
        )[0][0]

        breakthrough = self.db.execute_query(
            f"SELECT `{col}`, AttributeId FROM `breakthrough_attribute` WHERE ID = {character_id}"
        )
    ...
    def get_weapon_base_stats(self, weapon_name: str, level: int) -> Dict[str, Any]:
        ...
        w_info = self.db.execute_query(
            f"SELECT ID, type, Rarity FROM `weapon` WHERE Name = '{weapon_name}'"
        )
        if not w_info:
            raise ValueError(f"Weapon {weapon_name} not found.")
        wid, wtype, rarity = w_info[0]

        atk = self.db.execute_query(f"SELECT `{col}` FROM `w_atk` WHERE ID = {wid}")[0][
            0
        ]
        sub_info = self.db.execute_query(
            f"SELECT `{col}`, AttributeId FROM `w_secondary_attribute` WHERE ID = {wid}"
        )
