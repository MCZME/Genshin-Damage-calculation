...
class AutomationManager:
    ...
    async def add_character(self, name: str, region: str = "OTHER") -> bool:
        """自动化添加一个新角色。"""
        get_emulation_logger().log_info(f"--- 开始自动化处理角色: {name} ---", sender="Automation")
        
        # 1. 抓取数据
        char_id = self.fetcher.find_avatar_id(name)
        if not char_id: return False
        ...
        if not real_id:
            get_emulation_logger().log_error("数据库同步失败，停止文件生成", sender="Automation")
            return False
        ...
        # 5. 生成文件
        get_emulation_logger().log_info(f"正在生成本地代码文件: {output_path}", sender="Automation")
