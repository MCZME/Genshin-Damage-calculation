...
class DatabaseSync:
    ...
    ATTR_NAME_TO_ID = {
        "暴击率": 1, "暴击伤害": 2, "生命值": 3, "防御力": 4, "攻击力": 5, 
        "元素精通": 6, "元素充能效率": 7, "治疗加成": 8, "元素伤害加成": 9, 
        "受治疗加成": 10, "生命值%": 11, "攻击力%": 12, "防御力%": 13
    }
    ...
    def sync_character(self, data: Dict[str, Any]) -> Optional[int]:
        ...
        get_emulation_logger().log_info(f"开始同步数据库资产: {char_name}", sender="DBSync")
        ...
        try:
            ...
            get_emulation_logger().log_info(f"数据库同步成功 (Real ID: {real_id})", sender="DBSync")
            return real_id
        except Exception as e:
            get_emulation_logger().log_error(f"数据库同步失败: {str(e)}", sender="DBSync")
            return None
        ...
        for table in tables:
            for col in ["95", "100"]:
                ...
                if not self.db.execute_query(check_sql):
                    alter_sql = f"ALTER TABLE `{table}` ADD COLUMN `{col}` DOUBLE DEFAULT 0"
    ...
    def _sync_stats_table(self, table_name: str, char_id: int, stats_data: Dict[int, Dict], key_name: str) -> None:
