...
class TestDamagePipeline(unittest.TestCase):
    ...
    def test_snapshot_attributes(self):
        ...
        self.assertEqual(self.ctx.stats['base_atk'], 1000)
        self.assertEqual(self.ctx.stats['crit_rate'], 50.0)
        self.assertEqual(self.ctx.stats['damage_bonus'], 0.0)

    def test_calculate_math_simple(self):
        # 模拟快照
        self.ctx.stats['base_atk'] = 1000
        self.ctx.stats['def_mult'] = 0.5
        self.ctx.stats['res_mult'] = 0.9 # 10% res -> 0.9 multiplier
    ...
    def test_add_bonus(self):
        self.ctx.add_modifier("damage_bonus", 50.0)
        self.assertEqual(self.ctx.stats['damage_bonus'], 50.0)
