...
class TestAuraTheoryRigorous:
    ...
    @pytest.mark.parametrize(
        "atk_el, atk_u, target_el, target_u, expected_rem_target_g, expected_type",
        [
            (
                Element.HYDRO,
                1.0,
    ...
    def test_amplifying_clash(
        ...
        assert all(
            r.category == ReactionCategory.AMPLIFYING
            for r in results
            if r.reaction_type == expected_type
        )

        if expected_rem_target_g > 0:
            assert manager.auras[0].current_gauge == pytest.approx(
                expected_rem_target_g
            )
        else:
    ...
    @pytest.mark.parametrize(
        "atk_el, target_el, expected_type",
        [
    ...
    def test_ec_double_swirl(self, manager):
        ...
        swirl_targets = [
            r.target_element
            for r in results
            if r.reaction_type == ElementalReactionType.SWIRL
        ]
