...
class TestReactionSystemUnit:
    ...
    def test_transformative_reaction_dispatch(
        self, reaction_sys, sim_ctx, source_entity, target_entity
    ):
        ...
        sim_ctx.event_engine.publish(
            GameEvent(
                ...
                data={"target": target_entity, "elemental_reaction": res},
            )
        )

        # 3. 验证 ReactionSystem 是否截获该事件并发布了名为 "超载" 的剧变伤害事件
        assert any(
            isinstance(e.data.get("damage"), Damage) and e.data["damage"].name == "超载"
