...
class FurinaCenterOfAttentionEffect(BaseEffect):
    ...
    def __init__(self, owner: Any, duration: int = 600):
        super().__init__(owner, "万众瞩目", duration=duration, stacking_rule=StackingRule.REFRESH)
    ...
    def handle_event(self, event: GameEvent):
        if self.remaining_hits <= 0: return
    ...
    def _apply_flat_damage(self, event: GameEvent):
        ...
        tags = AttackTagResolver.resolve_categories(dmg_ctx.config.attack_tag, dmg_ctx.config.extra_attack_tags)
        if not (AttackCategory.NORMAL in tags or AttackCategory.CHARGED in tags or AttackCategory.PLUNGING in tags):
            return
    ...
    def _process_hit_effect(self, event: GameEvent):
        ...
        if not dmg or dmg.source != self.owner: return
        
        from core.action.attack_tag_resolver import AttackTagResolver, AttackCategory
        tags = AttackTagResolver.resolve_categories(dmg.config.attack_tag, dmg.config.extra_attack_tags)
        if not (AttackCategory.NORMAL in tags or AttackCategory.CHARGED in tags or AttackCategory.PLUNGING in tags):
            return
        ...
        if self.owner.ctx.space and self.owner.ctx.space.team:
            for m in self.owner.ctx.space.team.get_members():
                self.owner.event_engine.publish(GameEvent(
                    ...
                    source=self.owner, 
                    data={"character": self.owner, "target": m, "amount": m.current_hp * 0.01, "ignore_shield": True}
                ))
