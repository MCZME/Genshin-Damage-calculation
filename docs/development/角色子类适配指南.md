...
### 2.3 效果文本资源 (`ui/assets/descriptions.py`)
效果描述文本集中存放于 `ui/assets/descriptions.py`，与核心仿真引擎解耦。

**映射规则**：
- **字典键**：效果名称（中文，如 `"普世欢腾"`），需与 `simulation_effect_lifecycles` 表中的 `name` 字段一致。
- **模板语法**：支持 Python 格式化字符串 `{metric_key:format_spec}`，动态数值从 `simulation_mechanism_metrics` 表注入。

**示例**：
```python
EFFECT_DESCRIPTIONS: dict[str, str] = {
    # 芙宁娜 - 普世欢腾
    # "气氛值" 来自 simulation_mechanism_metrics 表的 metric_key
    "普世欢腾": (
        "累计气氛值，提升造成的伤害与受治疗加成。\n"
        "当前气氛值: {气氛值:.0f}"
    ),
...
**新增效果描述流程**：
1. 在 `ui/assets/descriptions.py` 的 `EFFECT_DESCRIPTIONS` 字典中添加条目。
2. 键名必须与仿真引擎记录的效果名称完全一致（检查 `effects.py` 中的注册名）。
3. 若需动态数据，确认该数据已通过 `ctx.record_mechanism_metric()` 写入 `simulation_mechanism_metrics` 表。
