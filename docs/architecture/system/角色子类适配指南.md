...
UI 通过类方法感知参数 Schema，实现无码化渲染。角色类必须实现 `get_action_metadata()` 类方法。

### 6.1 动作 Key 映射表 (UI ID to Internal Key)
UI 使用的动作 ID 与元数据字典中的 Key 必须遵循以下映射：

| UI 动作 ID | 元数据 Key | 说明 |
| :--- | :--- | :--- |
| `normal` | `normal_attack` | 普通攻击 |
| `charged` | `charged_attack` | 重击 |
| `plunging` | `plunging_attack` | 下落攻击 |
| `skill` | `elemental_skill` | 元素战技 |
| `burst` | `elemental_burst` | 元素爆发 |
| `dash` | `dash` | 冲刺 |
| `jump` | `jump` | 跳跃 |
| `skip` | `skip` | 等待/跳过帧 (通用) |

### 6.2 参数定义字段 (Parameter Schema)
`params` 列表中的每个参数项需包含：
...
- **`options`**: (Optional) 仅 `select` 类型有效，格式为 `{"value": "Label"}`。
...
---
*版本: V2.4.1 (Action Metadata Spec)*
*更新日期: 2026-02-13*
