...
## 2. 数据流向 (Data Flow)
1.  **探测阶段 (Discovery)**: UI 调用实体的 `get_action_metadata()` 获取参数 Schema。
2.  **配置阶段 (UI)**: 用户根据 Schema 录入意图参数（如选择“长按”），存储于 `PrototypeState`。
3.  **持久化阶段 (File I/O)**: 
...
```mermaid
graph TD
    subgraph Discovery [探测层 - Schema Discovery]
        D1[Backend: Character/Skill] -- "get_action_metadata()" --> D2[UI: Dynamic Inspector]
    end

    subgraph Frontend [前端配置层 - NiceGUI]
        A[用户交互界面] -- "根据 Schema 录入" --> B[PrototypeState /意图参数/]
    ...
    subgraph Assembly [后端解析层 - Parser]
        C -- "sequence_config" --> E[ActionParser]
        E -- "结构验证" --> H[ActionCommand /意图指令/]
    end

    subgraph Core [引擎驱动层 - Core Engine]
        H -- "Simulator 调度" --> I[ActionManager]
        I -- "Resolve Intent" --> J[Skill: to_action_data]
        J -- "产出物理块" --> K[ActionFrameData /判定帧/位移/]
        K -- "驱动帧循环" --> L[仿真状态 /Snapshot/]
    end

    subgraph Persistence [持久化与复盘层 - V3.1]
        L -- "take_snapshot()" --> M[DataProjector]
        M -- "多轨投影" --> N[(SQLite: 5 Tracks Storage)]
        N -- "追溯+补丁" --> O[ReviewDataAdapter]
        O -- "合成快照" --> P[UI: Analysis View]
    end
...
## 6. 仿真结果持久化传输 (Persistence Data Flow - V3.1)

为了支持高性能的战果复盘与精准审计，仿真产出的状态流采用 **“多轨投影 (Multi-Track Projection)”** 架构进行传输与存储。

### 6.1 投影分流机制 (The Projector)
`Simulator` 产出的原始快照 (Snapshot) 由 `DataProjector` 进行业务拆解，分流至五个动态轨道：
1.  **Pulse (脉搏轨)**: 记录角色的坐标、动作 ID、在场状态。采用增量算法（位移 > 0.01 触发写入）。
2.  **Jump (跳变轨)**: 记录 HP、能量的阶梯式突变。
3.  **Lifecycle (生命周期轨)**: 记录修饰符 (Modifier) 和效果 (Effect) 的存续区间。通过 `modifier_id` 实现精准闭合。
4.  **Mechanism (机制轨)**: 自动扫描 `custom_metrics` 字典，记录机制特化数值（如气氛值）。
5.  **Event (事件轨)**: 离散行为的结构化存储（含四级联动：Log、Data、Audit、Payload）。

### 6.2 唯一标识体系 (Identity System)
*   **Modifier ID / Instance ID**: 确保每一个增益或效果实例在 Session 内拥有唯一“身份证”，解决同名叠加的审计歧义。
*   **Event ID**: 由数据库自增，作为行为明细与原始负载的关联锚点。

### 6.3 还原与消费逻辑 (Recovery)
`ReviewDataAdapter` 采用 **“追溯+补丁”** 模式：
*   **定位**: 锁定目标帧 $T$。
*   **追溯**: 获取 $\le T$ 时刻最新的 Pulse 和 Jump 记录。
*   **应用**: 将覆盖 $T$ 帧的所有活跃 Lifecycle 补丁实时应用到静态基准面板上，还原出完整的视觉与逻辑快照。

---
*版本: V3.1 (Persistence Re-aligned)*
*日期: 2026-02-15*
