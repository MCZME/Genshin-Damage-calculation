...
*   **`BaseEntity`**: 实体基类。提供生命周期管理、空间属性（pos, facing, hitbox, faction）。
*   **`CombatEntity`**: 战斗实体。继承 BaseEntity，内置元素附着逻辑（Aura）与 ICD 控制。
*   **`AttackEntity`**: 攻击实体。轻量级攻击载体，支持多种触发模式（延迟、追踪、碰撞）。
*   **`SceneEntity`**: 场景实体。长期存在于场景中，支持区域检测和周期性逻辑。
...
---
*版本: v3.1.0 (Entity Architecture Extension)*
*日期: 2026-03-28*
