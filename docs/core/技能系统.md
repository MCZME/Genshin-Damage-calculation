...
### 1.2 元数据发现 (Metadata Discovery)
为了让 UI 能够动态生成参数编辑器，角色与技能系统必须暴露其参数模式。
- **`get_action_metadata()`**: 
    - **职责**: 返回该角色支持的动作及其对应的参数 Schema。
    - **Schema 结构**:
        ```python
        {
            "action_key": {
                "label": "显示名称",
                "params": [
                    {"key": "type", "label": "施放方式", "type": "select", "options": ["点按", "长按"]},
                    {"key": "index", "label": "段落", "type": "int", "min": 1, "max": 5}
                ]
            }
        }
        ```
