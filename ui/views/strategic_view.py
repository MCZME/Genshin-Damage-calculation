...
class StrategicView(ft.Container):
    ...
    def _build_ui(self):
        ...
        self.canvas = UniverseCanvas(self.state.universe_root, on_node_select=on_node_select)
        ...
        self.tabs_control = ft.Tabs(
            ...
            content=ft.Column([
                self.tab_bar,
                ft.Container(content=self.tab_views, expand=True)
            ], expand=True),
            expand=True
        )
    ...
    def _render_team(self):
        ...
        for char in self.state.team:
            ...
            self.team_list.controls.append(
                ft.Container(
                    content=ft.Row([
                        ft.Icon(ft.Icons.PERSON, color=ft.Colors.CYAN_700 if not is_active else ft.Colors.ORANGE_ACCENT, size=20),
                        ft.Text(c["name"], weight="bold", size=13, color=ft.Colors.WHITE if not is_active else ft.Colors.ORANGE_ACCENT),
                        ft.IconButton(ft.Icons.CLOSE, icon_size=14, on_click=lambda _, ch=char: self._remove_character(ch))
    ...
    def _render_targets(self):
        ...
        for target in self.state.targets:
            ...
            self.target_list.controls.append(
                ft.Container(
                    content=ft.Row([
                        ft.Icon(ft.Icons.G_TRANSLATE, color=ft.Colors.RED_700 if not is_active else ft.Colors.ORANGE_ACCENT, size=20),
                        ft.Text(target["name"], weight="bold", size=13, color=ft.Colors.WHITE if not is_active else ft.Colors.ORANGE_ACCENT),
                    ]),
                    padding=10,
                    bgcolor="#1a1a1a" if not is_active else "#252525",
                    border_radius=8,
