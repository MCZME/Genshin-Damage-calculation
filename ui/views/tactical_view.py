...
class TacticalView(ft.Row):
    ...
    def _build_ui(self):
        ...
        self.left_col = ft.Column([
            ft.Row([
                ft.Text("指令时间轴 (TIMELINE)", size=12, weight=ft.FontWeight.W_900, opacity=0.5),
                ft.IconButton(ft.Icons.DELETE_SWEEP_OUTLINED, icon_size=18, opacity=0.3, on_click=lambda _: self._clear_sequence())
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            
            # 动作库入口区
            ft.Container(
                content=self.library_container,
                height=70, bgcolor="rgba(255,255,255,0.02)", border_radius=12, padding=10
            ),
            
            # 滚动时间轴
            ft.Container(
                content=self.timeline,
                expand=True, bgcolor="rgba(0,0,0,0.1)", border_radius=16, padding=10
            )
        ], expand=3, spacing=15)
    ...
    def _render_library(self):
        ...
        if not active_members:
            self.library_container.controls.append(ft.Text("请先在战略阶段添加角色", size=11, italic=True, opacity=0.3))
            return
            ...
            for act_label, act_id in [("E", "skill"), ("Q", "burst"), ("A", "normal")]:
                self.library_container.controls.append(
                    ft.Container(
                        ...
                        on_click=lambda _, c=char["name"], a=act_id: self._add_action(c, a)
                    )
                )
            ...
            self.timeline.controls.append(
                ft.Container(
                    ...
                    content=ft.Row([
                        ...
                        ft.Text(action["char_name"], size=12, weight=ft.FontWeight.BOLD),
                        ft.Text(action["action_id"].upper(), size=11, opacity=0.6, expand=True),
                        ft.IconButton(ft.Icons.DELETE_OUTLINE, icon_size=16, opacity=0.3, on_click=lambda _, idx=i: self._remove_action(idx))
                    ]),
                    padding=12,
                    bgcolor="rgba(255,255,255,0.05)" if is_selected else "rgba(255,255,255,0.02)",
                    border_radius=10,
    ...
    def _render_inspector(self):
        ...
        if self.selected_index is None or self.selected_index >= len(self.state.action_sequence):
            self.inspector_content.controls.append(
                ft.Column([
                    ft.Icon(ft.Icons.AUTO_FIX_HIGH, size=40, opacity=0.1),
                    ft.Text("选择一个动作进行编辑", size=12, italic=True, opacity=0.3)
        ...
        self.inspector_content.controls.extend([
            ft.Text(f"编辑: {action['char_name']}", size=14, weight=ft.FontWeight.BOLD),
