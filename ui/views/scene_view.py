    ...
    def build(self, state: StrategicState):
        ...
        res_controls = cast(list[ft.Control], [
            StatInputField(
                ...
                on_select=lambda v, key=k: handle_resistance_change(key, v)
            ) for k in res_keys
        ])
        ...
        workbench_controls: list[ft.Control] = [
            upper_panel, 
            lower_section, 
        ...
        if is_picker_open:
            ...
            def finish_monster_select(mid: str):
                state.add_target(mid)
                # StrategicState.add_target 内部已调用 notify
