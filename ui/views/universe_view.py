...
class UniverseView(ft.Container):
    ...
    def _build_ui(self):
        ...
        self.hud = ft.Container(
            content=ft.Row([
                ...
                ft.Row([
                    ft.Container(
                        content=ft.Text("模拟样本: --", size=11, color=GenshinTheme.PRIMARY, weight=ft.FontWeight.BOLD),
                    ...
                    ft.ElevatedButton(
                        ...
                        bgcolor=GenshinTheme.PRIMARY, color=GenshinTheme.ON_PRIMARY,
                        on_click=self._handle_run_batch
                    ),
                    ft.Row([
                        ft.IconButton(ft.Icons.SAVE_OUTLINED, tooltip="保存批处理方案", on_click=self._handle_save_universe),
                        ft.IconButton(ft.Icons.FOLDER_OPEN_OUTLINED, tooltip="加载批处理方案", on_click=self._handle_load_universe),
                    ], spacing=0)
                ], spacing=20)
            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            padding=ft.padding.symmetric(horizontal=32, vertical=12), height=80,
            bgcolor=GenshinTheme.HEADER_BG, blur=ft.Blur(20, 20),
    ...
    def refresh(self):
        ...
        try:
            self.hud.content.controls[1].controls[0].content.value = f"模拟样本: {count}"
    ...
    def _handle_load_universe(self, e):
        ...
        def confirm(fname):
            ...
            self.page.pop_dialog()
        for f in files:
            lv.controls.append(ft.ListTile(title=ft.Text(f), on_click=lambda _, n=f: confirm(n)))
        self.page.show_dialog(ft.AlertDialog(
            title=ft.Text("加载方案"),
            content=ft.Container(lv, width=300)
        ))
