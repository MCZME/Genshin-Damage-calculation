        ...
        if not active_members:
            ...
            if not self.is_compact:
                self.controls.append(
                    ft.Text(
                        "可用动作库", size=10, weight=ft.FontWeight.W_900, opacity=0.4
                    )
                )
    ...
    def _build_character_group(self, char, color):
        ...
        if not self.is_compact:
            ...
            return ft.Container(
                ...
                tooltip=char_name,
                shadow=ft.BoxShadow(
                    blur_radius=10, color=ft.Colors.with_opacity(0.3, color)
                ),
            )
        ...
        return ft.Container(
            content=ft.Row(
                [
                    # 强化首字
                    ft.Text(
                        first_char, size=16, weight=ft.FontWeight.W_900, color=color
                    ),
                    ft.Text(
                        full_label[1:], size=11, weight=ft.FontWeight.BOLD, opacity=0.8
                    ),
                ],
                spacing=4,
                tight=True,
