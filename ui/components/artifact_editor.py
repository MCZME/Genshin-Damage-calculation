...
class ArtifactEditor(ft.Column):
    ...
    def _rebuild_controls(self):
                    ...
                    ft.Row(
                        [
                            ft.TextButton(
                                ...
                                on_click=self._handle_export_set,
                            ),
                            ft.TextButton(
                                "导入",
                                icon=ft.Icons.DOWNLOAD_FOR_OFFLINE,
                                on_click=self._handle_import_set,
                            ),
                        ],
                        spacing=0,
                    ),
        ...
        for key, initial, label, icon in slots:
            ...
            slot_data = arts.get(
                key, {"set": "未装备", "main": "生命值", "value": 0, "subs": []}
            )
    ...
    def _build_slot_block(self, key, initial, full_label, data):
        """构建高辨识度无边框部位块"""
        accent_color = GenshinTheme.get_element_color(
            self.char_data["character"]["element"]
        )
        ...
        for sub in data.get("subs", [])[:4]:
            ...
            sub_previews.append(
                ft.Text(
                    f"{sub['key']} +{sub['value']}{'%' if s_pct else ''}",
                    size=8,
    ...
    def _handle_block_hover(self, e, color):
        if e.data == "true":
            ...
            e.control.shadow = ft.BoxShadow(
                blur_radius=10, color=ft.Colors.with_opacity(0.1, color)
            )
        else:
    ...
    def _handle_export_set(self, _):
        name_input = ft.TextField(
            label="名称",
            value=f"{self.char_data['character']['name']}_预设",
            dense=True,
        )

        def confirm(_):
            if name_input.value:
                self.state.save_artifact_set_template(
                    self.char_data["artifacts"], name_input.value
                )
    ...
    def _handle_import_set(self, _):
        ...
        self.page.show_dialog(
            ft.AlertDialog(
                title=ft.Text("导入套装"), content=ft.Container(lv, width=300)
            )
        )

    def _open_slot_editor(self, key):
        data = self.char_data["artifacts"][key]
        implemented_sets = list(ArtifactSetMap.keys())
        ...
        for s_name in self.state.artifact_sets:
            ...
            set_options.append(
                ft.dropdown.Option(
                    key=s_name,
                    text=s_name if is_imp else f"{s_name}(待)",
                    disabled=not is_imp,
                )
            )
        set_drop = ft.Dropdown(
            label="套装",
            value=data["set"],
            options=set_options,
            dense=True,
        ...
        if data["main"] not in valid_mains:
            data["main"] = valid_mains[0]
        main_drop = ft.Dropdown(
            label="主词条",
            value=data["main"],
            options=[ft.dropdown.Option(m) for m in valid_mains],
            dense=True,
            expand=True,
        )
        main_val_field = ft.TextField(
            label="数值",
        ...
        sub_inputs = []
        sub_rows = []
        while len(data["subs"]) < 4:
            data["subs"].append({"key": "无", "value": 0.0})
        for i in range(4):
            sub_data = data["subs"][i]
            k_drop = ft.Dropdown(
                value=sub_data["key"],
                options=[ft.dropdown.Option("无")]
                + [ft.dropdown.Option(s) for s in self.SUB_STATS.keys()],
                dense=True,
                width=160,
            )
            v_field = ft.TextField(
                value=str(sub_data["value"]),
                dense=True,
                width=100,
                input_filter=ft.InputFilter(
                    allow=True, regex_string=r"[0-9\.]", replacement_string=""
            ...
            sub_inputs.append((k_drop, v_field))
            sub_rows.append(
                ft.Row(
                    [ft.Text(f"#{i + 1}", size=10, opacity=0.3), k_drop, v_field],
                    spacing=10,
                )
            )

        def save_and_close(e):
            data["set"] = set_drop.value
            data["main"] = main_drop.value
            data["value"] = float(main_val_field.value or 0)
            data["subs"] = []
            for k, v in sub_inputs:
                if k.value != "无":
                    data["subs"].append({"key": k.value, "value": float(v.value or 0)})
            self.page.pop_dialog()
            self.refresh()
            self.state.refresh()

        dialog = ft.AlertDialog(
            title=ft.Text(f"编辑 {key.upper()}"),
            content=ft.Container(
                content=ft.Column(
                    [
                        set_drop,
                        ft.Row([main_drop, main_val_field], spacing=10),
