...
class TimelineSequence(ft.Column):
    ...
    def refresh(self):
        ...
        self.controls.append(
            ft.Row(
                [
                    ft.Text(
                        "动作执行序列", size=12, weight=ft.FontWeight.W_900, opacity=0.5
                    ),
                    ft.Row(
                        [
                            ft.Text(
                                f"共 {len(self.state.action_sequence)} 步",
                                size=10,
                                opacity=0.3,
                            ),
                            ft.TextButton(
        ...
        self.controls.append(
            ft.Column([self.flow_container], expand=True, scroll=ft.ScrollMode.AUTO)
        )
    ...
    def _build_action_block(self, index, action):
        ...
        char_entry = next(
            (m for m in self.state.team if m and m["character"]["name"] == char_name),
            None,
        )
        color = GenshinTheme.get_element_color(
            char_entry["character"]["element"] if char_entry else "Neutral"
        )
        full_label, first_char = self.ACTION_META.get(
            action["action_id"], (action["action_id"].upper(), "?")
        )
                ...
                [
                    ft.Container(
                        bgcolor=ft.Colors.with_opacity(
                            0.15 if is_selected else 0.05, color
                        ),
                        border_radius=10,
                    ),
                    ft.Container(
    ...
    def _handle_drag_accept(self, e, target_idx):
        ...
        try:
            ...
            if not src_control or src_control.data is None:
                return
            ...
            # 范围校验
            if source_idx >= len(self.state.action_sequence):
                return
    ...
    def _remove_action(self, index):
        ...
        if 0 <= index < len(self.state.action_sequence):
            ...
            if self.state.selected_action_index == index:
                ...
                self.state.selected_action_index is not None
                and self.state.selected_action_index > index
            ):
