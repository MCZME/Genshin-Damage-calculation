...
class TargetPicker(ft.AlertDialog):
    ...
    def __init__(self, state: AppState, on_select: callable):
        ...
        self.title = ft.Text(
            "选择打击目标", weight=ft.FontWeight.BOLD, color=GenshinTheme.ON_SURFACE
        )
        ...
        self.content = ft.Container(width=400, height=400, content=self.list_container)
        ...
        for name, info in self.state.target_map.items():
            self.list_container.controls.append(
                ft.ListTile(
                    ...
                    title=ft.Text(
                        name, weight=ft.FontWeight.BOLD, color=GenshinTheme.ON_SURFACE
                    ),
                    subtitle=ft.Text(
                        f"Lv.{info['level']}",
                        size=11,
