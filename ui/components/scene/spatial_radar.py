...
def SpatialRadar(
    ...
    """
    # --- 辅助工厂 ---
    def create_ring(radius_m: float, label: str | None = None) -> ft.Control:
        px = radius_m * scale
        ...
        if label:
            ring_controls.append(
                ft.Text(label, size=8, color=ft.Colors.WHITE_12, offset=ft.Offset(0, -px / scale))
            )
            
        return ft.Stack(controls=ring_controls, alignment=ft.Alignment.CENTER)

    # 1. 基础环与坐标轴
    rings: list[ft.Control] = [create_ring(float(m), f"{m}m") for m in [5, 10, 15]]
    axes: list[ft.Control] = [
    ...
    # 3. 目标点集
    target_dots: list[ft.Control] = []
    guideline: ft.Control = ft.Container()
    ...
    for i, vm in enumerate(target_vms):
        ...
        dot_size = 10.0 if is_active else 6.0
        # 防止除以 0 且适配 offset 计算
        off_row = (pos_x * scale) / dot_size
        off_col = (-pos_z * scale) / dot_size
    ...
    return ft.Container(
        content=ft.Stack(controls=stack_controls, alignment=ft.Alignment.CENTER),
        width=size, 
        height=size,
