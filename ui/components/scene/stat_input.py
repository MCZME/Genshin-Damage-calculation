...
def StatInputField(
    ...
    if label_options:
        label_ctrl = ft.Dropdown(
            ...
            on_select=lambda e: on_label_change(e.control.value) if on_label_change else None
        )
    else:
        label_ctrl = ft.Text(
            label, size=10, color=GenshinTheme.TEXT_SECONDARY, 
            weight=ft.FontWeight.W_400, style=ft.TextStyle(letter_spacing=0.5)
        )
    ...
    tf = ft.TextField(
        ...
        dense=True, content_padding=ft.Padding(5, 10, 5, 10), border=ft.InputBorder.NONE,
        cursor_color=elem_color, cursor_width=1,
    ...
    items = []
    if icon_ctrl: items.append(icon_ctrl)
    items.append(label_ctrl)
    items.append(tf)

    return ft.Container(
        content=ft.Row(items, alignment=ft.MainAxisAlignment.SPACE_BETWEEN, vertical_alignment=ft.CrossAxisAlignment.CENTER, spacing=0),
        width=width, height=38, padding=ft.Padding(10, 0, 10, 0), border_radius=6,
