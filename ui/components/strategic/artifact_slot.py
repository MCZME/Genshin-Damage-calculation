...
def SubStatRow(
    ...
    current_key = str(vm.sub_stats[index][0])
    current_val = str(vm.sub_stats[index][1])
...
def ArtifactSlot(
    vm: ArtifactPieceViewModel,
    set_options: list[str], # 由外部 (LibraryVM) 传入选项列表
    element: str = "Neutral",
    ...
    main_stat_options = {
        ...
        "Goblet": [
            "攻击力%", "生命值%", "防御力%", "元素精通", 
            "火元素伤害加成%", "水元素伤害加成%", "风元素伤害加成%", "雷元素伤害加成%", 
            "草元素伤害加成%", "冰元素伤害加成%", "岩元素伤害加成%", "物理伤害加成%"
        ],
    ...
    name_dropdown = ft.Dropdown(
        ...
        on_select=lambda e: vm.set_set_name(e.control.value or "")
    )
    ...
    main_stat_drop = ft.Dropdown(
        ...
        on_select=lambda e: vm.set_main_stat(e.control.value or "")
    )
    ...
    main_val_input = ft.TextField(
        ...
        border=ft.InputBorder.OUTLINE,
        border_color=ft.Colors.with_opacity(0.1, ft.Colors.WHITE),
        on_change=lambda e: vm.set_main_val(e.control.value or "")
    )

    # 4. 副词条列表
    sub_options = [
        "生命值", "生命值%", "攻击力", "攻击力%", "防御力", "防御力%", 
        "元素精通", "元素充能效率%", "暴击率%", "暴击伤害%"
    ]
    
    substat_items: list[ft.Control] = [
    ...
    main_col_controls: list[ft.Control] = [
        ...
        ft.Row(controls=[main_stat_drop, main_val_input], spacing=5),
        ...
        ft.Column(controls=substat_items, spacing=5),
        ft.Row(controls=[add_sub_btn], alignment=ft.MainAxisAlignment.CENTER)
    ]

    return ft.Container(
        content=ft.Column(controls=main_col_controls, spacing=5),
