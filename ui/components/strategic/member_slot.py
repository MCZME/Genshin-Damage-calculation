...
def MemberSlot(
    ...
    # --- 内部组件工厂 ---
    def create_mini_talent_tag(label: str, val: int):
        return ft.Container(
            content=ft.Column([
    ...
    remove_btn = ft.Container(
        ...
        on_click=lambda e: [on_remove(index) if on_remove else None],
    )
    ...
    weapon_info = f"{vm.weapon.display_name[:10]}  Lv.{vm.weapon.level}" if vm.weapon.name else "未装备武器"
    weapon_row = ft.Row([
        ft.Icon(ft.Icons.SHIELD, size=11, color=ft.Colors.with_opacity(0.45, ft.Colors.WHITE)),
        ft.Text(weapon_info, size=10, color=ft.Colors.with_opacity(0.65, ft.Colors.WHITE), no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS, expand=True)
    ], spacing=5)
    ...
    artifact_row = ft.Row([
        ...
        ft.Text(f"已装备 {artifact_count} 件圣遗物", size=10, color=ft.Colors.with_opacity(0.65, ft.Colors.WHITE), no_wrap=True, overflow=ft.TextOverflow.ELLIPSIS, expand=True)
    ], spacing=5)
