    ...
    def __init__(
        ...
        super().__init__(
            ...
            padding=ft.padding.symmetric(
                horizontal=12 if not is_compact else 4, vertical=8
            ),
            ...
            if not is_compact:
                self.content = ft.Row(
                    [
                        ...
                        ft.Text(
                            subtitle, size=11, color=color, opacity=0.8, expand=True
                        ),
                        ft.IconButton(
                            ft.Icons.CLOSE,
                            icon_size=14,
                ...
                self.content = ft.Container(
                    content=circle_node, alignment=ft.Alignment.CENTER
                )
        else:
            if not is_compact:
                self.content = ft.Row(
                            ...
                            [
                                ft.Row(
                                    [
                                        ...
                                        ft.Text(
                                            element_icon
                                            if isinstance(element_icon, str)
                                            else "",
                                            size=10,
                                ...
                                ft.Text(
                                    subtitle, size=11, color=GenshinTheme.TEXT_SECONDARY
                                ),
                            ],
                            spacing=2,
                            expand=True,
...
class EntityPool(ft.Column):
    ...
    def refresh(self):
        ...
        if not self.is_compact:
            self.controls.append(
                ft.Row(
                    [
                        ft.Text(
                            "队伍配置", size=10, weight=ft.FontWeight.W_900, opacity=0.4
                        ),
                        ft.Text(
                            f"{len([m for m in self.state.team if m])}/4",
                            size=10,
                            opacity=0.3,
                        ),
                    ],
        ...
        for i, member in enumerate(self.state.team):
            ...
            team_col.controls.append(
                SelectorItem(
                    title=char_info["name"],
                    subtitle=f"Lv.{char_info['level']} {char_info['type']}",
                    icon=ft.Icons.PERSON,
                    element_icon=char_info["element"],
                    color=color,
        ...
        if len([m for m in self.state.team if m]) < 4:
            try:
                next_idx = self.state.team.index(None)
            except ValueError:
                next_idx = len(self.state.team)
            team_col.controls.append(
                AddButton(
                    "添加角色",
                    lambda _: self.state.select_character(next_idx),
        ...
        if not self.is_compact:
            self.controls.append(
                ft.Text("怪物目标", size=10, weight=ft.FontWeight.W_900, opacity=0.4)
            )
        ...
        target_col.controls.append(
            AddButton(
                "新建目标",
                lambda _: self.state.add_target(),
        ...
        is_env_selected = (
            self.state.selection and self.state.selection.get("type") == "env"
        )
        env_icon = ft.Icon(
            ft.Icons.CLOUD,
            size=18 if not self.is_compact else 20,
            ...
            ft.Container(
                content=ft.Row(
                    [
                        env_icon,
                        ft.Text(
                            "环境配置", size=11, weight=ft.FontWeight.BOLD, expand=True
                        ),
                        ft.Icon(ft.Icons.CHEVRON_RIGHT, size=16, opacity=0.5),
                    ]
                )
                if not self.is_compact
                else ft.Container(content=env_icon, alignment=ft.Alignment.CENTER),
                ...
                border=ft.border.all(
                    1, ft.Colors.CYAN_200 if is_env_selected else "transparent"
                ),
