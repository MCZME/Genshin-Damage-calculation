    ...
    def __init__(self, node, is_selected, on_click, on_add):
        ...
        super().__init__(
            content=ft.Stack([
                ft.Column([
                    ft.Row([
                        ft.Text(node.name or "宇宙变体", size=11, weight=ft.FontWeight.BOLD, expand=True, 
                                color=ft.Colors.AMBER_100 if is_managed else GenshinTheme.ON_SURFACE),
                        ft.IconButton(
                            ft.Icons.ADD_CIRCLE_OUTLINE, icon_size=14, 
                            on_click=lambda _: on_add(node), opacity=0.6, 
                            icon_color=GenshinTheme.PRIMARY,
                            visible=True # 允许受控节点添加后续分支
                        )
                    ], spacing=5),
                    ft.Text(node.rule.label if node.rule else ("BASE" if node.id == "root" else "Inherited"), 
...
class UniverseTreeContainer(ft.Stack):
    ...
    def __init__(self):
        ...
        self.node_layer = ft.Stack(width=8000, height=8000, clip_behavior=ft.ClipBehavior.NONE)
...
class UniverseCanvas(ft.GestureDetector):
    ...
    def _draw_collect(self, node, x_depth, y_idx, nodes_list, lines_list):
        is_selected = (self.state.selected_node and self.state.selected_node.id == node.id)
        pos_x = 100 + x_depth * 250
        pos_y = 100 + y_idx * 120
        
        ui_node = UniverseNodeUI(node, is_selected, self.state.select_node, self.state.add_branch)
