...
class MutationInspector(ft.Container):
        ...
        if node.id != "root" and not node.is_managed and not has_children:
            self.rules_area.controls.append(
                ft.TextButton(
                    ...
                    if not node.rule
                    else ft.Icons.EDIT_NOTE_ROUNDED,
                    ...
                    ft.Row(
                        [
                            ...
                            ft.IconButton(
                                ft.Icons.CLOSE,
                                icon_size=14,
                                on_click=lambda _: self._delete_rule(),
                                visible=not self.state.selected_node.is_managed,
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ),
                    ft.Text(
                        " > ".join(map(str, rule.target_path)),
                        size=10,
                        opacity=0.5,
                        color=GenshinTheme.ON_SURFACE,
                    ),
    ...
    def _show_add_rule_dialog(self, e):
        ...
        if current_config:
            ...
            for i, member in enumerate(team):
                if member:
                    char_options.append(
                        ft.dropdown.Option(
                            key=str(i), text=f"#{i + 1} {member['character']['name']}"
                        )
                    )
        ...
        char_drop = ft.Dropdown(
            label="目标位置",
            options=char_options,
            value="0" if char_options else None,
            visible=False,
        )
        category_drop = ft.Dropdown(
            label="分类",
            options=[ft.dropdown.Option(k) for k in targets.keys()],
            on_select=lambda e: self._handle_category_change(
                e, property_drop, char_drop, targets
            ),
        )
        ...
        is_range = ft.Switch(
            label="开启区间扫描模式",
            value=False,
            on_change=lambda _: self._toggle_mode(is_range, value_input, range_fields),
        )
        ...
        def on_confirm(_):
            ...
            if is_range.value:
                ...
                rule = ModifierRule(
                    target_path=path,
                    value=val,
                    label=f"[{idx + 1}] {property_drop.value}",
                )
    ...
    def _handle_category_change(self, e, property_drop, char_drop, targets):
        cat = e.control.value
        property_drop.options = [ft.dropdown.Option(p) for p in targets[cat]]
        property_drop.disabled = False
        char_drop.visible = cat in ["角色属性", "武器属性"]
        property_drop.update()
        char_drop.update()

    def _map_to_path(self, category, prop, idx):
        m = {
            "角色属性": ["level", "constellation"],
            "武器属性": ["level", "refinement"],
        }
        field = prop  # 默认
        if category == "角色属性":
        ...
        if category in ["角色属性", "武器属性"]:
            path.extend(
                [
                    "team",
                    idx,
                    "character" if category == "角色属性" else "weapon",
                    field,
                ]
            )
        else:
    ...
    def _handle_name_change(self, e):
        if self.state.selected_node:
            self.state.update_node(self.state.selected_node.id, name=e.control.value)

    def _handle_sync_click(self, e):
        config = self.state.get_selected_node_config()
        if config:
            self.state.branch_to_main.put(
                {
                    "type": "APPLY_CONFIG",
                    "config": config,
    ...
    def _delete_rule(self):
        if self.state.selected_node:
            self.state.update_node(self.state.selected_node.id, rule=None)
