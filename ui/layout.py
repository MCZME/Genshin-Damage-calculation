        ...
        with ui.header().classes('genshin-glass border-b border-white/5 items-center justify-between px-8 py-3'):
            with ui.row().classes('items-center gap-6'):
                ...
                with ui.row().classes('bg-white/5 p-1 rounded-full border border-white/5'):
                    ui.button('战略', icon='settings').props('flat rounded size=sm color=primary').classes('px-6 text-[10px] font-black tracking-widest')
                    ui.button('战术', icon='timeline').props('flat rounded size=sm color=white').classes('px-6 text-[10px] font-black tracking-widest opacity-30')
                    ui.button('复盘', icon='analytics').props('flat rounded size=sm color=white').classes('px-6 text-[10px] font-black tracking-widest opacity-30')
