...
def sync_issue_to_board(issue_number, target_status):
    ...
    if not issue_url_raw: return
    issue_url = json.loads(issue_url_raw)['url']

    # 2. 将 Issue 添加到项目并获取 Item ID
    item_add_raw = run_command(f"gh project item-add {PROJECT_NUMBER} --owner {PROJECT_OWNER} --url {issue_url} --format json")
    if not item_add_raw: return
